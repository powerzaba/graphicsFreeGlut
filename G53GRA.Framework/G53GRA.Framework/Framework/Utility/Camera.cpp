...
void Camera::Update(const double& deltaTime)
{
	...
	//Move towards looking direction
	if (wKey && !isInNewPos)
		add(eyePosition, vd, speed);

	//Move backwards, negative of looking direction
	if (sKey && !isInNewPos)
		sub(eyePosition, vd, speed);
	...
	if (mKey && !isInNewPos) {
		...
		eyePosition[0] = newPosition[0];
		eyePosition[1] = newPosition[1];
		eyePosition[2] = newPosition[2];
